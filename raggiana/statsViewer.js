...
(function(parent) {
  ...
  var defaultMetrics = [
    ...
    "client/request_latency_ms_p99",
  ];
  parent.Raggiana = {
    readProvidedData : function(data, callback) {
      finalData = [];
      var inputData = Raggiana.parseContents(data, "INF");
    ...
    selectDefaultMetrics : function() {
      for (var index = 0; index < defaultMetrics.length; ++index) {
        $('[id$="' + defaultMetrics[index] + '"]').click();
      }
    },
    ...
    readSingleFile : function(evt) {
      ...
      if (f) {
        ...
        r.onload = function(e) {
         try {
            readProvidedData(e.target.result)
          } catch (err) {
  ...
  */
  $(document).ready(function() {
    ...
    // Load default stats when page is loaded! Added by michael.zalimeni@bronto.com
    var defaultData = $("#_default-data").text().trim();
    if (defaultData) {
      Raggiana.readProvidedData(defaultData, function(){Raggiana.selectDefaultMetrics()});
    }
  });
})(window);
